...
async function fetchZeptoPrices(query, pincode = "110078") {
  ...
  try {
    ...
    const products = await page.evaluate(sel =>
      Array.from(document.querySelectorAll(sel)).map(card => ({
        name: card.querySelector("[data-testid='product-card-name']")?.innerText.trim() || "",
        quantity: card.querySelector("[data-testid='product-card-quantity']")?.innerText.trim() || "",
        price: card.querySelector("[data-testid='product-card-price']")?.innerText.trim() || "",
        originalPrice: card.querySelector("p.line-through")?.innerText.trim() || "",
        deliveryTime: document.querySelector("span.font-extrabold")?.innerText.trim().replace(/\s+/g, " ") || ""
      }))
    , productCard);
